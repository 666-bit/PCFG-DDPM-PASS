...
def sample_run(args, checkpoint_dic, pcfg):
    with open(args.input_dir +"/"+ 'charmap.pickle', 'rb') as f:
        charmap = pickle.load(f)

    with open(args.input_dir  +"/"+ 'inv_charmap.pickle', 'rb') as f:
        inv_charmap = pickle.load(f)
    ...
    for i in tqdm(range(args.num_samples)):
        ...
        for _ in range(args.seq_length):
            ...
            if inv_charmap[int(predicted2)] != "<END>" and inv_charmap[int(predicted2)] != "unk":
                word = ""
                for _ in range(5):
                    _word = gen_pcfg(output_word,checkpoint_dic,pcfg, args)
                    if _word:
                        word += "".join(_word).replace('`', '')
                        break
                if word != "":
                    password += word
            else:
...
def parse_args():  
    ...
    # 输入目录  
    parser.add_argument('--input_dir', type=str, default="../data", help='Input directory path') 
    ...
    # 输出文件  
    parser.add_argument('--output', type=str, default="lstm_gan.txt", help='Output file path')  
  
    # 样本数量  
    parser.add_argument('--num_samples', type=int, default=1000000, help='Number of samples to generate')  
  
    # 检查点文件  
    parser.add_argument('--checkpoint', type=str, default="LSTM_checkpoints/6-2-PCFG-model-64-6-16.h5", help='Checkpoint file path')  
