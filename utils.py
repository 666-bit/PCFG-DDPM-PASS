...
def load_pcfg_dataset(path, max_length, tokenize=False, max_vocab_size=2048):
    lines = []
    ...
    # startChar = "<BEG>"
    # endChar = "<END>"
    # charmap = {'unk':0, startChar:1, endChar:2}
    # inv_charmap = ['unk',startChar,endChar]
...
def save_vocab(charmap, inv_charmap, output_dir):
    with open(f"{output_dir}/charmap.pickle", 'wb') as f:
        pickle.dump(charmap, f)
    with open(f"{output_dir}/charmap_inv.pickle", 'wb') as f:
        pickle.dump(inv_charmap, f)
    print("Character map saved to {}".format(output_dir))

def decode_samples(samples, inv_charmap, unk_marker='<UNK>'):
    decoded_samples = []
    for sample in samples:
        decoded = [inv_charmap[int(c)] if inv_charmap[int(c)] != 'unk' else unk_marker for c in sample.squeeze().cpu().numpy()]
