...
def train_run(args):
    ...
    lines, charmap, inv_charmap = utils.load_pcfg_dataset(
    ...
    gradient_penalty = tf.reduce_mean((slopes)**6)  # WGAN-div
    ...
    with tf.Session(config=config) as session:
        ...
        def generate_samples():
            ...
            for i in range(len(samples)):
                ...
                for j in range(len(samples[i])):
                    decoded.append(inv_charmap[samples[i][j]])
                decoded_samples.append(" ".join(tuple(decoded)))
            return decoded_samples
        ...
        for iteration in range(args.iters):
            ...
            if iteration % args.save_every == 0 and iteration > 0:
                ...
                model_saver.save(session, (args.output_dir +"/"+ 'checkpoints' +"/"+ f'checkpoint_{iteration}.ckpt'))
